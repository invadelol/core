...
class CompressionService {
   ...
   */
  async compress(match: RiotAPITypes.MatchV5.MatchDTO): Promise<CompressedMatch> {
    if (!match || typeof match !== 'object') throw new TypeError('compress() needs an object')
  ...
  async decompress(input: CompressedMatch): Promise<RiotAPITypes.MatchV5.MatchDTO> {
  ...
  private toShort = (v: any): any =>
    Array.isArray(v)
      ? v.map(this.toShort)
      : v && typeof v === 'object'
        ? Object.fromEntries(Object.entries(v).map(([k, val]) => [MAP[k] ?? k, this.toShort(val)]))
        : v
  ...
  private toLong = (v: any): any =>
    Array.isArray(v)
      ? v.map(this.toLong)
      : v && typeof v === 'object'
        ? Object.fromEntries(Object.entries(v).map(([k, val]) => [REV[k] ?? k, this.toLong(val)]))
        : v
}
